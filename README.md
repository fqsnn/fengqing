...
- 普通对话受控调用智能体：检查、测试和项目分析可自动委派，代码写入仍需明确授权
- 本机私人记忆管理：原生窗口中可查看、新增、修正和删除
- 持久活动历史：统一记录智能体执行与记忆变更
- 持久任务状态机：任务会记录计划、运行、验证与完成或待处理状态
- 本机 workspace 动态同步：Markdown 修改后在下一轮对话生效
...
WORKSPACE_CONTEXT_DIR=C:/path/to/read-only-workspace
```
...
当前自动委派使用明确、可审计的意图路由。它不是后台无限自治：普通检查和测试可以自动运行，修改代码必须勾选“允许写入”，所有执行都会进入活动历史。
