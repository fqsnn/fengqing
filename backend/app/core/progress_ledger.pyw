...
MILESTONES: list[JsonMap] = [
    ...
    {"name": "本机 Python 受控执行", "proof": "内置工具将批准的代码模板写入临时目录、隔离运行，并返回真实标准输出与退出码，不触碰项目源码"},
    {"name": "本机可视化 Python 工具", "proof": "Python 在本地生成 PNG 运行产物；原生客户端仅显示受控运行目录中的图像，并拒绝越界路径"},
    {"name": "受控联网检索", "proof": "只发送本次明确检索词，带超时、来源链接与失败说明；私人 workspace 和记忆不外发"},
    {"name": "项目推进直达执行", "proof": "明确继续推进自己的项目时，智能体执行受控自编程而非只解释路线"},
    {"name": "意图兜底保护", "proof": "能力问句有明确答复；未知请求先澄清，绝不默认伪装成项目分析"},
    {"name": "本地模型真实诊断", "proof": "状态接口用带超时的只读探针确认 Ollama 连通性、配置模型和已下载模型，不把配置值伪装成运行事实"},
    {"name": "可执行资源均衡", "proof": "请求按复杂度、风险和操作类型选择上下文、输出、反思与规划预算；公开创作资料按相关性取回，预算和原因可审计"},
]

CURRENT = {
    "stage": "AI 核心第四轮：均衡调度",
    "focus": "让对话、反思、智能体规划和代码写入按复杂度与风险使用不同资源，并把选择写成可审计证据",
    "next": "用真实会话持续校准各档预算，优先修复错误路由、无效上下文和验证缺口",
}
