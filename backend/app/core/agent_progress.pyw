...
def summarize_agent_progress(plan: list[JsonMap], results: list[JsonMap], allow_write: bool) -> JsonMap:
    ...
    return {
        ...
        "summary": _summary(steps, passed, allow_write),
        "steps": steps,
        "passed": passed,
        "next": _next(passed, allow_write),
    }
...
def _summary(steps: list[JsonMap], passed: bool, allow_write: bool) -> str:
    write = "允许写入" if allow_write else "未写入"
    return f"本轮智能体执行 {len(steps)} 步，状态：{'通过' if passed else '需要处理'}，模式：{write}。"


def _next(passed: bool, allow_write: bool) -> str:
    if not passed:
